...
class VPNServer(threading.Thread):
    ...
    def __init__(self):
        ...
        iff_flag = self.IFF_TUN | self.IFF_NO_PI
        ifr = struct.pack('16sH', settings.IF_NAME, iff_flag)
        ioctl(self.tun, self.TUNSETIFF, ifr)
        ioctl(self.tun, self.TUNSETOWNER, 1000)
        ifup = 'sudo ifconfig {0} {1} {1} netmask 255.255.255.0 up'
        ifup = ifup.format(settings.IF_NAME, settings.GATEWAY_ADDR)
        subprocess.check_call(ifup, shell=True)
        ...
        while (True):
            ...
            for x in xrange(count):
                if x:
                    ...
                    req = dns.requests.rx.Initialize(data=dl[x],
                                                     count=count, id=id)
                    data.append(req)
    ...
    def run(self):
        ...
        while (True):
            ...
            if req.qd.qtype == dns.type.NS:
                ...
                pprint(data)
            ans = DNSRR(rrname=req.qd.qname,
                        ttl=1, rdata=rdata, type=req.qd.qtype)
